...
static bool should_use_affinity = false;
...
void mojoexec_make_bigcore_affine() {
    ...
    if(result != 0) {
        printf("MojoExec: setting affinity failed: %s\n", strerror(result));
    }else{
        printf("MojoExec: forced current thread onto big core\n");
        big_core_affine = true;
    }
}

JNIEXPORT void JNICALL
Java_git_artdeell_mojoexec_MojoExec_setUseBigCoreAffinity(JNIEnv *env, jclass clazz, jboolean enable) {
    should_use_affinity = enable;
}